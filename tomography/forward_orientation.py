...
N_GRAIN = 100
PROJECT_NAME = fr"forward_orientation_map_grains_{N_GRAIN}"
...
orientation_data = xr.DataArray(
    ...
    coords={
        "y": np.linspace(y0, y1, hsv_img.shape[0]),  # rows → y
        "x": np.linspace(x0, x1, hsv_img.shape[1])   # cols → x
    },
...
ele_per_wavelength_ls = np.arange(1,13,1)
# ele_per_wavelength_ls = [4]


model_order = 4

for elements_per_wavelength in ele_per_wavelength_ls:
