...
PROJECT_NAME = fr"tomography_cubic_solid_heterogeneous_isotropic"
...
VP = np.sqrt( (matl.C13 + 2*matl.C55) / RHO )
VS = np.sqrt(matl.C55 / RHO)
...
C13 = matl.rotated_tensor(np.pi/6, dim=2)[0,2]
C55 = matl.rotated_tensor(np.pi/6, dim=2)[2,2]

VP = np.sqrt( (C13 + 2*C55) / RHO )
# VS = np.sqrt(C55 / RHO)


material_oriented = sn.material.elastic.Velocity.from_params(rho=RHO, vp=VP, vs=VS)
...
def tx_rx_pos(n, x1, y1, tol=2e-3):
...
# id = 2
# f_adj_0 =  (data_true[id] - data_init[id]) 

# fig, axs = plt.subplots(1, 3, figsize=(20, 6), dpi=100)


# axs[0].plot(time*1e6, data_true[id])
# axs[0].plot(time*1e6, data_init[id])
