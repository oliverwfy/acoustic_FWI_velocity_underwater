...
PROJECT_NAME = f"tomography_heterogeneous_circle_phi_src_32_smooth_0mm_roi_7mm"
...
homogeneous_model = lm.LayeredModel(
    [
        sn.material.acoustic.Velocity.from_params(rho=RHO, vp=VP),
    ]
)
...
defect_mask = np.linalg.norm(centroids - np.array([x1/2, y1/2]), axis=1) < 5e-3
...
sources = [
    ...
    #     x=tx[0], y=tx[1], 
    #     fx=nomal_vec[_i,0], fy=nomal_vec[_i,1]
    #     ) 
    # for _i, tx in enumerate(tx_coordinates)
        sn.simple_config.source.cartesian.ScalarPoint2D(
        x=tx[0], y=tx[1], 
        f=1
        ) 
    for _i, tx in enumerate(tx_coordinates)
